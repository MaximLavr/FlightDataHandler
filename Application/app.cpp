...
private:
    ...
    Label* logLatLon = nullptr;
    Label* logTime = nullptr;;
    Label* logAlt = nullptr;;
    Label* logGPSAlt = nullptr;;
    Label* logHead = nullptr;;
    Label* logPitch = nullptr;;
    Label* logRoll = nullptr;;
    Label* logVelocity = nullptr;;
    Label* logDoubles;
...
public:
    ...
    {
        if (logLatLon == nullptr) {
            logLatLon = new Label("Coords (lat,lon):",
                                  subWindow->m_PaddingLeft,
                                  subWindow->m_Size.y - subWindow->m_HeaderHeight
                                  - subWindow->m_PaddingUp
                                  - float(1) * FontManager::get("Lusia_UI")->GetHeight("W")
                                  - float(0) * subWindow->m_StringInterval,
                                  "Lusia_UI", transform_color(1, 1, 1, 1));
            subWindow->AddDataText(logLatLon);
        }
        if (logTime == nullptr) {
            logTime = new Label("Time (sec):",
                                  subWindow->m_PaddingLeft,
                                  subWindow->m_Size.y - subWindow->m_HeaderHeight
                                  - subWindow->m_PaddingUp
                                  ...
                                  "Lusia_UI", transform_color(1, 1, 1, 1));
            subWindow->AddDataText(logTime);
        }
        if (logAlt == nullptr) {
            logAlt = new Label("Alt (m):",
                                  subWindow->m_PaddingLeft,
                                  subWindow->m_Size.y - subWindow->m_HeaderHeight
                                  - subWindow->m_PaddingUp
                                  ...
                                  "Lusia_UI", transform_color(1, 1, 1, 1));
            subWindow->AddDataText(logAlt);
        }
        if (logGPSAlt == nullptr) {
            logGPSAlt = new Label("GPS alt (m):",
                                  subWindow->m_PaddingLeft,
                                  subWindow->m_Size.y - subWindow->m_HeaderHeight
                                  - subWindow->m_PaddingUp
                                  ...
                                  "Lusia_UI", transform_color(1, 1, 1, 1));
            subWindow->AddDataText(logGPSAlt);
        }
        if (logHead == nullptr) {
            logHead = new Label("Head (deg):",
                                  subWindow->m_PaddingLeft,
                                  subWindow->m_Size.y - subWindow->m_HeaderHeight
                                  - subWindow->m_PaddingUp
                                  ...
                                  "Lusia_UI", transform_color(1, 1, 1, 1));
            subWindow->AddDataText(logHead);
        }
        if (logPitch == nullptr) {
            logPitch = new Label("Pitch (deg):",
                                  subWindow->m_PaddingLeft,
                                  subWindow->m_Size.y - subWindow->m_HeaderHeight
                                  - subWindow->m_PaddingUp
        ...
        if (logVelocity == nullptr) {
            logVelocity = new Label("Velocity (m/sec):",
                                  subWindow->m_PaddingLeft,
                                  subWindow->m_Size.y - subWindow->m_HeaderHeight
                                  - subWindow->m_PaddingUp
                                  ...
                                  "Lusia_UI", transform_color(1, 1, 1, 1));
            subWindow->AddDataText(logVelocity);
        }
    }
    ...
    void init() override
    {
        ...
        //layer3 = new Layer(new BatchRenderer2D(window), shader_map1, ortho, true);
    ...
    void update(TimeStep ts) override {
        ...
        if (flightDataHandlerButton->isPresed() && map == nullptr && !loaded) {
            ...
            map = new Map(mapPosX, mapPosY, mapSize,
            ...
            if (map->m_Track != nullptr)
            {
                ...
                for (auto& i : map->m_Track->m_Renderables)
                {
                    ...
                    if (con1 && con2 && con3 && con4 && con5 && con6 && !zoomInButton->isHovered() && !zoomOutButton->isHovered()) {
                        if (window->isMouseButtonPressed(GLFW_MOUSE_BUTTON_1) && (previousPointChoosed != i))
                        {
                            pointAdded = false;
                            if (!pointAdded)
                            {
                            ...
                            pointLayer->add(new Sprite(x - circleSize / 2.0f,
                                                       ...
                                                       new Texture("", "../images/circle.bmp")));
                            map->pointedTrackItem = i;
                            allData = map->m_Track->GetInfoFromPixelCoords(i,
                                                                           map->m_Window,
                                                                           map->mapLayer,
                                                                           map->m_TileSide,
                                                                           map->getSize().x,
                            ...
                            spriteImpulseGraph->SetTexture(new Texture("", "../images/graphics_test/impulse_graph.jpg"));
                            ...
                            //
                            spriteImpulseHist->SetTexture(new Texture("", "../images/graphics_test/impulse_hist.jpg"));

                            spriteAltGraph->setColor(transform_color(1, 1, 1, 1));
                            if (spriteAltGraph->m_Texture != nullptr)
                                spriteAltGraph->m_Texture->deleteTexture();
                            //
                            spriteAltGraph->SetTexture(new Texture("", "../images/graphics_test/alt.jpg"));
                            ...
                            //
                            spriteSurfTypeGraph->SetTexture(new Texture("", "../images/graphics_test/surface_type.jpg"));

                            logSurfaceType->text = "Water";
#if 0
                            graphics = nullptr;
                        //graphics = new Graphics(map->m_Track->times, map->m_Track->alts, map->m_Track->impulses);
...
{
private:
    float windowWidth = 2500; //2500
    float windowHeight = 1563; //1563
    float cameraRight = windowWidth / 60.0f;
    float cameraLeft = -cameraRight;
    float cameraUp = windowHeight / 60.0f;
    float cameraBottom = -cameraUp;
...
public:
    ...
    void addSprites(unsigned int color)
    {
        for (float i = cameraLeft; i < cameraRight; i += 1.0f)
        {
            for (float j = cameraBottom ; j < cameraUp; j += 1.0f)
            {
                layer1->add(new Sprite(i, j, 0, 0.8f, 0.8f, color));
            }
        }
    }
    ...
    void freeSprites()
    {
        for (auto& sprite : layer1->m_Renderables->getArr())
        {
    ...
    void init() override
    {
        ...
        shader_map1 = new Shader("../SparkyEngine/src/shaders/basicTexNoLight.vert","../SparkyEngine/src/shaders/basicTexNoLight.frag");
        //shader_map1 = new Shader("../SparkyEngine/src/shaders/test/shader1.vert","../SparkyEngine/src/shaders/test/shader1.frag");


        mat4 ortho = mat4::orthographic(-window->getWidth() / 60.0f, (window->getWidth() / 60.0f), -(window->getHeight() / 60.0f), (window->getHeight() / 60.0f), -1.0f, 1.0f);

        texturePath = "/Users/maxim_lavrinenko/Documents/projects/FlightDataHandlerDIST/images/MainMap/15_zoom/15_21872_9912.png";
        layer1 = new MapLayer(new BatchRenderer2D(window), shader_map1, ortho, true);
        addSprites(transform_color(0.2f, 0.3f, 0.8f, 1.0f));
        //sprite1 = new Sprite(-5, -5, 0, 25, 25, new Texture("", texturePath));
        sprite1 = new Sprite(-5, -5, 0, 0.8, 0.8, transform_color(1, 1, 1, 1));
        //layer1->add(sprite1);
    ...
    void update() override {
        if (window->isKeyPressed(GLFW_KEY_W))
        {
            random = rand() % 3;
            /*if (random == 0)
                //sprite1 = new Sprite(-5, -5, 0, 25, 25, new Texture("", "/Users/maxim_lavrinenko/Documents/projects/FlightDataHandlerDIST/images/MainMap/15_zoom/15_21872_9912.png"));
                sprite1 = new Sprite(-5, -5, 0, 25, 25, transform_color(1, 0, 1, 1));
            else if (random == 1)
                sprite1 = new Sprite(-5, -5, 0, 25, 25, transform_color(0.2f, 0, 1, 1));
                //sprite1 = new Sprite(-5, -5, 0, 25, 25, new Texture("", "/Users/maxim_lavrinenko/Documents/projects/FlightDataHandlerDIST/images/MainMap/15_zoom/15_21872_9911.png"));
            else if (random == 2)
                sprite1 = new Sprite(-5, -5, 0, 25, 25, transform_color(0.2f, 0.3f, 0.8, 1));
                //sprite1 = new Sprite(-5, -5, 0, 25, 25, new Texture("", "/Users/maxim_lavrinenko/Documents/projects/FlightDataHandlerDIST/images/MainMap/15_zoom/15_21871_9912.png"));*/

            //layer1->remove_all();
            freeSprites();
            std::cout << "reallocatinos!" << std::endl;
            //freeSprites();
            //addSprites(transform_color(0.2f, 0.3f, 0.8f, 1));
            addSprites(transform_color(0.2f, 0.3f, 0.8f, 1));
            //layer1->add(sprite1);
        }
    }
...
int main()
{
    std::string dirPath = "/Users/maxim_lavrinenko/Documents/projects/FlightDataHandlerDIST/Data/Data_19.05.07/Alapaevsk_18_09_21/data";
#if 0
    std::string fileName = "/Users/maxim_lavrinenko/Documents/projects/FlightDataHandlerDIST/Data/Data_19.05.07/NTagil_18_12_21/data/bsi00000.mat";

    std::ofstream oFile;

    oFile.open("/Users/maxim_lavrinenko/Documents/projects/FlightDataHandlerDIST/test_read.txt");
    ...
    matvar = Mat_VarRead(matfp,"B_Head");
    if ( NULL != matvar ) {
        //Mat_VarPrint(matvar, 1);
        double* matData = (double*)(matvar[0].data); //read array
        double ans = (double)matData[0];
    ...
    //auto paths = GetAllPathInDirectory(dirPath);

    //std::string outFilePath = "/Users/maxim_lavrinenko/Documents/projects/FlightDataHandlerDIST/Data/Track/TrackInfo.txt";
    ...
    //{
        ...
        trackLoader = new TrackLoader(dirPath);

        //double data1 = trackLoader->GetDataOneDouble("/Users/maxim_lavrinenko/Documents/projects/FlightDataHandlerDIST/Data/Data_19.05.07/NTagil_18_12_21/data/bsi00002.mat", "B_Head");
        ...
        mat_t    *matfp;
        matvar_t *matvar;
        matfp = Mat_Open("/Users/maxim_lavrinenko/Documents/projects/FlightDataHandlerDIST/Data/Data_19.05.07/NTagil_18_12_21/data/bsi00002.mat", MAT_ACC_RDONLY);
        if ( NULL == matfp ) {
            return EXIT_FAILURE;
        }
        matvar = Mat_VarRead(matfp,"B_Head");
