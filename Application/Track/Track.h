...
namespace app_graphics
{
    ...
    public:
        ...
        void Draw(vec2 latLong, int zoom, unsigned int color, vec2 mapPosition, Window* window, vec2_i leftUpTileCoords);
        void AddPoint(vec2 latLong, vec2 mapPos, float mapSize, int zoom, unsigned int color, Window *window, size_t pointerSize, vec2_i leftUpTileCoords);
        std::vector<double> GetInfoFromPixelCoords(Renderable2D* trackPoint, Window* window,
